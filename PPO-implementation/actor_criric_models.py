...
class ActorNwk(nn.Module):
    def __init__(self,input_dim,out_dim,
        ...
        self.actor_nwk = nn.Sequential(
            ...
            nn.LeakyReLU(),
            nn.Linear(hidden3_dim,hidden4_dim),
            nn.LeakyReLU(),
            nn.Linear(hidden4_dim,hidden5_dim),
            nn.LeakyReLU(),
            nn.Linear(hidden5_dim,hidden6_dim),
            nn.LeakyReLU(),
            nn.Linear(hidden6_dim,out_dim),  
            nn.Softmax(dim=-1)
        )
